...
FinishPage.prototype.render = function (_parent) {
    ...
    var downloadButton = $("<button id='downloadResults' data-role='button' data-inline='true'>" + "Download Results (CSV)" + "</button>");
    downloadButton.bind("click", (function(event, ui) {
      const csv = this.createCSV(this.session.trials);
      const link = document.createElement('a');
      link.setAttribute('href', csv);
      link.setAttribute('download', 'listening_test_results.csv');
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    }).bind(this));
    _parent.append(downloadButton);
...
// FinishPage.prototype.createCSV = function(trials) {
//   let csv = 'data:text/csv;charset=utf-8,';
//   csv += 'TestID,PageID,StimulusID,Rating,TimeSpent(ms)\n';
  
//   trials.forEach(trial => {
//     trial.responses.forEach(response => {
//       csv += `${trial.id},${trial.type},${response.stimulus},${response.score || response.stimulusRating},${response.time || ''}\n`;
...
//   // Add participant data headers
//   csv += 'ParticipantName,ParticipantAge,HearingLoss,HearingAid,PlaysGuitar,Consent,';
//   // Add test data headers
//   csv += 'TestID,PageID,StimulusID,Rating,TimeSpent(ms)\n';

//   // ✅ Use participant data directly from session
//   const participantData = this.session.participantData || {};

//   // Add one line for each response (skip consent trials)
//   trials.forEach(trial => {
//     if (trial.type === "consent") return;

//     trial.responses.forEach(response => {
//       csv += `"${participantData.name || ''}",`;
//       csv += `${participantData.age || ''},`;
//       csv += `${participantData.hearingLoss || ''},`;
//       csv += `${participantData.hearingAid || ''},`;
//       csv += `${participantData.playsGuitar || ''},`;
//       csv += `${participantData.consent || ''},`;
//       csv += `${trial.id},${trial.type},${response.stimulus},${response.score || response.stimulusRating},${response.time || ''}\n`;
//     });
//   });
...
FinishPage.prototype.createCSV = function(trials) {
  ...
  // Add headers including participant info
  csv += 'ParticipantName,ParticipantAge,HearingLoss,HearingAid,PlaysGuitar,TrackID,StimulusID,Rating,TimeSpent(ms)\n';

  // Get participant data
  const consentData = (this.session.participant && this.session.participant.consentData) || {};

  const participantFields = [
    `"${consentData.name || ''}"`,
    `${consentData.age || ''}`,
    `${consentData.hearingLoss || ''}`,
    `${consentData.hearingAid || ''}`,
    `${consentData.playsGuitar || ''}`,
  ];

  // Append one row per response
  trials.forEach(trial => {
    trial.responses.forEach(response => {
      const trialFields = [
        trial.id,
        response.stimulus,
        response.score || response.stimulusRating,
        response.time || ''
      ];
      csv += participantFields.concat(trialFields).join(',') + '\n';
    });
  });
